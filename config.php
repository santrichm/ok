...
if ($from_id == $admin || $userInfo['isAdmin'] == true) {
    $mainKeys = array();
    $temp = array();
    $mainKeys[] = [['text'=>"🆓🟢 تست رایگان ۱ روزه 🟢🆓",'callback_data'=>"getTestAccount"]];
    $mainKeys[] = [['text'=>"🎁موجودی رایگان",'callback_data'=>"inviteFriends"],['text'=>"💰کیف پول",'callback_data'=>"myInfo"]];
    ...
    $keys[] = [['text'=>"🆓🟢 تست رایگان ۱ روزه 🟢🆓",'callback_data'=>"getTestAccount"]];
    $keys[] = [['text'=>"🎁موجودی رایگان",'callback_data'=>"inviteFriends"],['text'=>"💰کیف پول",'callback_data'=>"myInfo"]];
    ...
    foreach($serverIp as $server_ip){
        if($inbound_id == 0) {
            if($protocol == 'vless'){
                ...
                $outputlink = "$protocol://$uniqid@$server_ip:" . ($rahgozar == true?"$port":$port) . "?type=$netType&security=" . ($rahgozar==true?"tls":$tlsStatus) . "{$psting}#$remark";
                if($netType == 'grpc'){
                    if($tlsStatus == 'tls'){
            ...
            if($protocol == 'vless'){
                ...
                if($netType == 'tcp'){
                    ...
                    if($rahgozar == true)$outputlink = "$protocol://$uniqid@$server_ip:$port?type=$netType&security=tls&path=" . urlencode("$path?ed=2048") . "&encryption=none&host=$server_ip{$psting}#$remark";
