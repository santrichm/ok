...
if (preg_match('/^\/([Ss]tart)/', $text) or $text == '⤵️ برگرد به منوی اصلی ' or $text == '🔙بازگشت به منوی اصلی' or $data == 'mainMenu') {
    ...
    if(isset($data) and $data == "mainMenu"){
        $res = editText($message_id, 'سلااام به ربات کوبی وی پی ان خوش اومدی 🫡🌸
...
', $mainKeys);
        if(!$res->ok){
            sendMessage('سلااام به ربات کوبی وی پی ان خوش اومدی 🫡🌸
...
if($userInfo['step'] == "addNewAdmin" && $from_id === $admin && $text != $cancelText){
    if(is_numeric($text)){
        ...
        sendMessage("عدد خود را به انگلیسی وارد کنید.");
    }
}
...
if($userInfo['step'] == "editInviteAmount"){
    if(is_numeric($text)){
        ...
        sendMessage("✅ تنظیمات بازاریابی",$keys);
        setUser();
    }else sendMessage("عدد خود را به انگلیسی وارد کنید.");
}
...
if($data=="increaseMyWallet"){
    delMessage();
    sendMessage("🙂 عزیزم مقدار شارژ مورد نظر خود را به تومان وارد کن و توجه کن که جهت واریز ارز دیجیتال باید مبلغی که وارد میکنی بیشتر از 150تومن و برای پرداخت به صورت کارت به کارت حداقل مبلغ واریزی باید 10تومن باشه.",$cancelKey);
    setUser($data);
}
if($userInfo['step'] == "increaseMyWallet" && $text != $cancelText){
    ...
    elseif($text < 10000){
        sendMessage("لطفا مبلغی که وارد میکنی بیشتر از 10هزار تومن و برای واریز ارز دیجیتال بیشتر از 150 تومن باشه");
        exit();
    }
...
if(preg_match('/increaseWalletWithCartToCart/',$data)) {
    ...
    delMessage();  
    setUser($data);
    sendMessage("🦋عزیزم برای افزایش موجودی در ربات از طریق روش کارت به کارت به دلایل مسائل امنیتی لطفا به آیدی پشتیبان پیام دهید تا باهاتون هماهنگ کنیم .
    
👤 <code>{$paymentKeys['bankAccount']}</code> - {$paymentKeys['holderName']}

در اولین وقت باهاتون هماهنگ میشه و پشتیبان ۲۴ ساعته پاسخگوست👀",$cancelKey, "HTML");
    exit;
}
...
در اولین وقت باهاتون هماهنگ میشه و پشتیبان ۲۴ ساعته پاسخگوست👀
",$cancelKey,"html");
    exit;
}
